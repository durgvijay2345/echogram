...
function Profile() {
    ...
    if (!profileData || !postData || !userData) {
        return (
        ...
        <div className='w-full min-h-screen bg-black'>
                    ...
                    <div className='flex items-center justify-center gap-[20px]'>
                        <div className='flex relative'>
                            {profileData.followers?.slice(0, 3).map((user, index) => (
                                <div key={user._id} className={`w-[40px] h-[40px] border-2 border-black rounded-full cursor-pointer overflow-hidden ${index > 0 ? `absolute left-[${index * 10}px]` : ""}`}>
            ...
            <div className='w-full min-h-[100vh] flex justify-center'>
                <div className='w-full max-w-[900px] flex flex-col items-center rounded-t-[30px] bg-white relative gap-[20px] pt-[30px] pb-[100px]'>
                    ...
                    {/* Posts Rendering */}
                    {profileData._id === userData._id && postData && Array.isArray(postData) && (
                        <>
                            {postType === "posts" && postData.map((post) => (
                                post.author?._id === profileData._id && <Post key={post._id} post={post} />
                            ))}
                            {postType === "saved" && userData.saved?.map((savedPostId) => {
                                const savedPost = postData.find(p => p._id === savedPostId);
                                return savedPost ? <Post key={savedPost._id} post={savedPost} /> : null;
                            })}
                        </>
                    )}
